...
import { WaterDropIcon, SunIcon, ThermometerIcon, SparklesIcon, TrashIcon, CalendarIcon, WarningIcon, ClockIcon, PencilIcon, DocumentTextIcon } from './Icons';

interface PlantCardProps {
  plant: Plant;
  onWater: (id: string) => void;
  onGetAiTip: (plant: Plant) => void;
  onDelete: (id: string) => void;
  onUpdateNotes: (id: string, notes: string) => void;
}

export const PlantCard: React.FC<PlantCardProps> = ({ plant, onWater, onGetAiTip, onDelete, onUpdateNotes }) => {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-lg hover:shadow-2xl dark:shadow-black/20 overflow-hidden flex flex-col transition-all duration-500 transform hover:-translate-y-2 group ${cardBorderClass}`}>
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="mt-4 space-y-3 text-gray-600 dark:text-gray-400 flex-grow">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          ...
          <button
            onClick={() => onGetAiTip(plant)}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-all duration-300"
          >
